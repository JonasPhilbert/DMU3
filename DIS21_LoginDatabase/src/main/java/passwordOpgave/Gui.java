...
public class Gui extends Application {
	...
	public void ButtonClicked(ActionEvent e) {
		if (e.getSource() == btnLogin) {
			if (db.matchPassword(brugernavn.getText(), password.getText())) {
				thestage.setScene(scene2);
			} else {
				lblBesked.setText("Login fejl.");
			}

		} else if (e.getSource() == btnOpret) {
			db.addUser(brugernavn.getText(), password.getText());
			password.clear();
			brugernavn.clear();
			lblBesked.setText("Bruger oprettet.");
